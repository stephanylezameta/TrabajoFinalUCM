...
def render_home() -> None:
    ...
    st.markdown('<div class="chat-main"><div class="chat-main-title">Asistente operativo</div></div>', unsafe_allow_html=True)
...
def render_tdrs_sidebar_controls() -> dict:
    """Controles laterales del TDRS.

    Mantiene la V04 y recupera únicamente el bloque **Pesos del modelo** sobre
    las restricciones. Los valores iniciales proceden del escenario superior
    activo y pueden ajustarse manualmente mediante sliders.
    """
...
def render_tdrs_scenario_selector() -> None:
    ...
    with st.container(border=True):
        ...
        st.markdown(
            f'<div class="selector-active"><strong>{meta["icon"]} {escape(current)}</strong> · {escape(meta["description"])}</div>',
            unsafe_allow_html=True,
        )
...
def render_tdrs(controls: dict) -> None:
    ...
    for idx, row in enumerate(rank_rows[:3]):
        ...
        podium.append(
            ...
            f'<div class="podium-rank">{medals[idx]} opción {idx+1}</div>'
            f'<div class="podium-name">{escape(destination)}</div>'
            f'</div>'
            f'<div class="podium-price">{escape(price_text)}</div>'
            f'</div>'
            f'<div class="podium-meta">Cobertura original {row["Cobertura"]} · {escape(visitors_text)}</div>'
            '</div>'
        )
    if podium:
