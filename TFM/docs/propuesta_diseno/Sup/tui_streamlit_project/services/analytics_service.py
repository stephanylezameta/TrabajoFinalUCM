...
def get_dashboard_metrics() -> dict:
    ...
    return {
        ...
        "cost_eur": cost, "roas": _safe_div(revenue, cost) if cost is not None else None,
...
def get_destination_metrics() -> list[dict]:
    return _repo.rows("""
        ...
        FROM events GROUP BY destination ORDER BY impressions DESC, clicks DESC
    """)
...
def instrumentation_status() -> list[dict]:
    ...
    return [
        ...
        {"KPI": "ROAS", "estado": "disponible" if m["cost_eur"] not in (None, 0) else "pendiente de instrumentar", "necesita": "cost_eur en bookings/campañas"},
